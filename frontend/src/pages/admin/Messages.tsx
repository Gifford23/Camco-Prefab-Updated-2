...
const Messages = () => {
  ...
  const readCount = contactMessages.filter(msg => msg.status === 'read').length;
  const archivedCount = contactMessages.filter(msg => msg.status === 'archived').length;
  const subscribedCount = newsletters.filter(sub => sub.status === 'subscribed').length;

  const exportToCSV = () => {
    const csvContent = newsletters.map(sub => 
      `${sub.email},${sub.source},${formatDate(sub.timestamp)},${sub.status}`
    ).join('\n');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100/50 p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Messages & Communications</h1>
          <p className="text-slate-600 mt-1">Manage customer inquiries and newsletter subscriptions</p>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-2">{readCount}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-2">{archivedCount}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-2">{subscribedCount}</p>
              </div>
                      ...
                      filteredMessages.map((message) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <Badge className={`${getStatusColor(message.status)} ml-2`} variant="secondary">
                              {message.status}
                            </Badge>
                          </div>
                          <div className="ml-11">
                      ...
                      <div className="bg-slate-50 p-4 rounded-lg">
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1">
                            <h4 className="font-bold text-lg text-slate-900">{selectedMessage.name}</h4>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50 hover:bg-slate-50">
                      ...
                      <TableHead className="font-semibold text-slate-700">Subscribed Date</TableHead>
                    ...
                    {filteredNewsletters.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={4} className="text-center py-16">
                          ...
                          <p className="text-lg font-medium text-slate-500">No newsletter subscriptions found</p>
