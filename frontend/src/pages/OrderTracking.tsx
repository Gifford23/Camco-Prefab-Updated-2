...
import { CheckCircle, Circle, Package, Truck, Clock, MapPin, Phone, Mail } from "lucide-react";
...
const OrderTracking = () => {
  ...
  const product = order ? products.find(p => p.id === order.products[0]?.productId) : null;
      ...
      <div className="container py-8 max-w-6xl">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-3">
            {isAuthenticated && <CustomerNotifications />}
                    ...
                    {statusSteps.map((step, index) => {
                      ...
                      return (
                        <div key={step.key} className="flex flex-col items-center text-center">
                          ...
                          <p className={`text-xs font-medium ${isCompleted || isCurrent ? 'text-gray-900' : 'text-gray-500'}`}>
                            {step.label}
                          </p>
                        </div>
                      );
