  ...
  const handleDownloadWord = () => {
    try {
      ...
      if (!content || content.trim() === '') {
        showNotification("No content to download. Please add content to the editor first.", "error");
        return;
      }
...
<body>${content}</body>
      ...
      link.href = url;
      link.download = `Contract_${contract.orderId}_${new Date().toISOString().split("T")[0]}.doc`;
      ...
      showNotification("Failed to download Word document. Please try again.", "error");
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Save the PDF
      pdf.save(`Contract_${contract.orderId}_${new Date().toISOString().split("T")[0]}.pdf`);
                  ...
                  <div className="flex gap-1 p-1 bg-slate-800/50 rounded-lg border border-slate-600/50">
                      ...
                      {showColorPicker && (
                        <div className="absolute top-full mt-2 bg-slate-800 border border-slate-600 rounded-lg shadow-2xl z-50 p-3">
                          <div className="grid grid-cols-5 gap-2">
                            {['#000000', '#1d4ed8', '#dc2626', '#16a34a', '#ea580c', '#9333ea', '#0891b2', '#ca8a04', '#64748b', '#ffffff'].map(color => (
                              <button
                                key={color}
                    ...
                    <div className="relative">
                      ...
                      {showBgColorPicker && (
                        <div className="absolute top-full mt-2 bg-slate-800 border border-slate-600 rounded-lg shadow-2xl z-50 p-3">
                          <div className="grid grid-cols-5 gap-2">
                            {['transparent', '#fef3c7', '#dbeafe', '#dcfce7', '#fee2e2', '#f3e8ff', '#e0f2fe', '#fef9c3', '#f1f5f9', '#ffffff'].map(color => (
                              <button
                                key={color}
                                ...
                                title={color}
                              >
                                {color === 'transparent' && <span className="text-xs text-slate-400">⊘</span>}
                              </button>
                            ))}
                          </div>
                        </div>
